...
## 記憶（memory）— ローカル LLM 向け

記憶の読み書きは **MCP ツール**（`.mcp.json` のサーバー名は `memory`）。`Skill(...)` では呼べない。

| やりたいこと | 正しいツール名 |
|--------------|----------------|
| 最近一覧 | `mcp__memory__list_recent_memories` |
| 文脈想起 | `mcp__memory__recall` |
| 検索 | `mcp__memory__search_memories` |
| 保存 | `mcp__memory__remember` |

**禁止**（`Unknown skill` になる）:

- `Skill(memory-mcp:...)` / `Skill(mcp_memory_mcp_...)` — パッケージ名やアンダースコア連結はスキル名ではない
- `memory-mcp` をサーバー名にしたツール名（正しくは `mcp__memory__*`）

最短経路: スラッシュコマンド **`/memories`**（`allowed-tools` で memory MCP のみ許可）。

`/permissions` で `mcp__memory__*` を許可しておく。`memory-mcp` プロセスは Claude Code セッション起動時に MCP として載る（HTTP recall 用に常駐している場合もある）。
...
> **ma-home 本番は Windows ネイティブ**（WSL2 不要）。以下は本家 README 由来のメモ。
