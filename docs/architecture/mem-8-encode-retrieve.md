...
| **MEM-8h** | cue-driven memory bridge — [mem-8h-memory-bridge.md](../tracks/mem-8h-memory-bridge.md) | **A–C ✅** · **D 📋** |
...
1. **8h-D** — 8a fact 行 + bridge 統合 · plan soft must_include → [mem-8h-memory-bridge.md](../tracks/mem-8h-memory-bridge.md)
2. **8g v1** — compose topic retire gist 本線 → [compose-topic-retire.md](../tracks/compose-topic-retire.md)
3. **8d** — L0 `answer_facts`、毎ターン全文 recall 禁止の整理
4. **8a** — 1 episode から fact 行を明示分離（ここっち型の再発防止）
