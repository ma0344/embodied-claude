...
細かい pan/tilt。**DS Cam / SS Desktop / SS Web(Edge) では PTZ 可・SS Web(Chrome) では不可**（ブラウザ相性）。直 ONVIF は API 4 原因（A〜D）別問題。CAM-1〜4 → [backlog-ma-home.md#cam--tapo-ptz--onvif-細かい操作が効かない合意-2026-06-20](./backlog-ma-home.md)。
...
4 層（WM≈セッション → STM≈日次 → LTM → Deep/SOUL）と昇格パイプライン。**encode 時と retrieve 時で「重要」が一致しない** — 多視点保存・**表層自動 / 詳細は能動 recall** の多段階（MEM-8）。**生活・仕事の自己開示**は **MEM-8e v0**（`detect_self_disclosure` → STM + profile gist + 訂正時 LTM）。詳細 → [backlog-ma-home.md#mem--記憶層セッション跨ぎ--dreaming](./backlog-ma-home.md) / [MEM-8e](./backlog-ma-home.md#mem-8e--自己開示の広い-encode合意-2026-06-23)。
...
まーが「調べて」と言ったときの CC `WebSearch` 空結果・捏造 Sources。**仕様** → [ws-2-conversation-web-search.md](./ws-2-conversation-web-search.md)（WS-1〜2c **済**）。**将来**: **WS-5** 自発検索（「調べて」無し・文脈＋plan）→ [backlog-ma-home.md](./backlog-ma-home.md)（MEM-5j）。

## GAPI — Google Calendar / Drive（ma-home）

予定・共有ドキュメントへの読み取りアクセス。GAPI-0〜5 → [backlog-ma-home.md](./backlog-ma-home.md)（GAPI 節）。
...
| 音響 | `wpctl` / `pactl`（ユーザ `ma` の PipeWire セッション）— `amixer Master` は不可 |
| オーバーレイ | `koyori-audio-server` `:18791` ← script が `POST /volume-notify` → presence-ui が poll |

```bash
curl -sS http://127.0.0.1:18791/health
sudo acpi_listen   # Vol+/-
```

ソース: `scripts/koyori-kiosk/surface-volume.sh`, `koyori-audio-server.py`
...
診断: `koyori-diagnose-input-leap` / ma-home: `scripts/check-koyori-stack.ps1`（一括） / `scripts/ma-home-input-leap-server.ps1`（レガシー）

ログオン自動起動: `.\scripts\configure-input-leap-winget.ps1`（WinGet 3.0.2 サービス）。レガシー CLI: `install-input-leap-task.ps1`
