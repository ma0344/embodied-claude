...
**最終更新**: 2026-06-26（LW-READ v0、B4/V5 完了反映）  
...
| ~~**Windows キーボードをキオスク（Surface）で共有**~~ — PC の KB/マウスを Surface 入力に（Input Leap） | [V5](#v--ビジョン--未実装docsweb_ui_designmdexported-sessionmd-より) | **済** 2026-06-26 |
| ~~**サーバー用ターミナルウィンドウの非表示**~~ — ログオン時 Task 起動の cmd/PowerShell を隠す | [B4](#b--運用自動化) | **済** 2026-06-26 |

### 次の順（合意 2026-06-17・まー → **2026-06-26 更新**）

**北極星**: [ALIVE](#alive--生きてる感-縦スライス合意-2026-06-25)

1. **LW-READ v0 様子見**（今夜）→ **v1 GW-S1 判断**（明日）— [ALIVE-6](#alive--生きてる感-縦スライス合意-2026-06-25) / [LW-READ](#lw-read--読書状態機械合意-2026-06-26)
2. **K1** — こより自身のコード経路（急がない）

---

## 優先順（合意 2026-06-13 → **2026-06-26 更新**）

| 順 | トラック | 内容 | 状態 |
|----|---------|------|------|
| **★** | **ALIVE** | 生きてる感 — LW-READ v0 **運用中** → v1 GW-S1 → LW-7 / LW-5 | **北極星・進行中** → [ALIVE](#alive--生きてる感-縦スライス合意-2026-06-25) |
| **D** | Backlog 最新化 | このファイルを現実に合わせる | **随時** |
| **B** | 運用自動化 | Task 常駐・**B4 ターミナル非表示済** | **ほぼ完了**（B2 LM Studio 手動のみ） |
| **V** | Surface ビジョン | **V5 Input Leap 済** → V4 see_near / V1 感情色 等 | **部分済** → [V](#v--ビジョン--未実装docsweb_ui_designmdexported-sessionmd-より) |
| **C** | **部屋 UI（Native + Surface）** | `/` 殻 + キオスク UX | **C11 実戦 OK** → 磨きは任意 |
| **A4** | **能動届け（Outbound）** | A4b+/c+/f/g 実装・運用済み | **運用中** |
| **OL** | Open Loops / リマインド | OL1+OL2 実装済み | **運用確認** → [open-loops-reminders.md](./open-loops-reminders.md) |
| **OL5** | 予定消化で loop close | 活動語+完了語（GW-S1 候補） | **計画済** → [OL5](#ol5--予定消化で-loop-終了合意-2026-06-25) |
| **GW** | **GW-SILENT** | 黙考ルート — LW-READ PAUSE / OL5 | **計画済**（プロンプト済、**S1 未配線**）→ [GW-SILENT](#gw-silent--黙考ルートsilent-internal-turn合意-2026-06-25) |
| **A3** | Gateway 直実行 | see / observe / autonomous-tick / 青空 | **済・運用中** → [A3](#a3--gateway-直実行身体自律) |
...
| **OBS** | **能動観察 `/observe`** | slash 完遂不能 + gateway フェーズ化 | **計画済** → [OBS](#obs--能動観察observe-完遂不能--gateway-フェーズ化合意-2026-06-20) |
| **CAM** | **Tapo PTZ / ONVIF** | 細かい pan/tilt；Chrome NG 調査 | **調査** → [CAM](#cam--tapo-ptz--onvif-細かい操作が効かない合意-2026-06-20) |
| **EAR** | **耳（環境音）** | Surface マイク → social | **計画済** → [EAR](#ear--耳環境音--surface-マイク合意-2026-06-19) |
| **GAPI** | **Google Calendar / Drive** | 予定・共有ドキュメント読み取り | **計画済** → [GAPI](#gapi--google-calendar--drive合意-2026-06-23) |
| **VIS** | **VL 安定性** | corrupt 相関ログ・ntfy | **様子見** → [VIS](#vis--vision-healthvl-安定性相関ログ) |
...
| **高** | ~~**V5 Windows キーボードをキオスクで共有**~~ | **済** — Input Leap / `configure-input-leap-winget.ps1` → [koyori-input-sharing.md](./koyori-input-sharing.md) |
...
- [ ] **V3** 発話インジケーター
- [ ] **V4** see_near
- [x] **V5** **Windows キーボードをキオスク（Surface）で共有** — Input Leap（2026-06-26）
- [ ] **V6** 家族リスト UI
