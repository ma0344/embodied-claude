...
- **desire satisfaction telemetry**（content が `[desire:` 始まり）→ 常時 `do_not_surface`（`desire_satisfaction_telemetry`）；**書き込み停止**（`PRESENCE_DESIRE_LTM_SATISFACTION` 既定 OFF · sidecar でクールダウン）· **purge** `scripts/purge-telemetry-ltm.py`（2026-07-19）
- **VISION raw dump** → LTM remember 既定停止（`PRESENCE_VISION_LTM_REMEMBER=0` · ライブは `vision_prefetch`）；既存行は同 purge（2026-07-19）
