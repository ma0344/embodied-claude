...
- **literary agent passage**（`青空文庫で読んだ` / `青空『…` 等 · LW-READ encode）→ 読書 cue なしなら `do_not_surface`（`literary_passage_off_topic`）。「大丈夫」系 soft reply は compose recall 自体を skip（2026-07-18）
