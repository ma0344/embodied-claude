...
**状態**: ✅ **prep-1/2/3 + GAPI-7 書込 router 実線**（ma-home CLI smoke OK · 会話 create/patch）  
...
| **GAPI-7** | 会話から Calendar **create + patch** · `[calendar_write_result]` | ✅ router 実線（`PRESENCE_GAPI_CALENDAR_WRITE=1`） |
...
**prep-2 で確認済**: CLI create + patch（`[gapi-smoke]`）。**会話 gateway 配線は GAPI-7（`calendar_write.py`）で実線**。
...
**次の一手**: 「今日の予定は？」会話 E2E（`PRESENCE_GAPI_ENABLED=1`）→ **GAPI-7**（会話からの書込）
