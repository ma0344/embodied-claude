...
**状態**: ✅ **prep-1/2/3 + GAPI-7a/7b 書込 router 実線**（ma-home CLI smoke OK · 会話 create/patch + 確認フロー）  
...
| **GAPI-2** | Calendar **読取** prefetch（today+tomorrow） | 1 | ✅ CLI · ✅ router（prep-3） |
...
**実装順**: GAPI-0 → GAPI-1 → **GAPI-2 + GAPI-5** → **GAPI-7a/7b** → **GAPI-7c** / **GAPI-7d**（任意）→ GAPI-4 → GAPI-6 → GAPI-3

**次の一手**: 7b 運用（正規化 affirm + 表層ガード）→ 必要なら 7d
