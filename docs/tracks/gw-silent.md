...
**状態**: ✅ S1 配線済（tick PAUSE）· ✅ S2 実装済（ingest OL-GATE、既定 OFF）· ✅ Claude resume（opt-in）
...
| ID | 内容 | 状態 |
|----|------|------|
| GW-S1 | `run_silent_internal_turn(session_id, task, schema)` | ✅ 2026-06-25（tick: LM Studio + stable append）· ✅ 2026-06-29 Claude `--resume`（opt-in） |
| GW-S1-resume | 表 chat 直後 background internal（同一 JSONL session） | ✅ 2026-06-29（`gw_resume.py` + `native_chat_router`） |
