...
**状態**: ✅ S1 配線済（tick PAUSE）· 📋 S2 / Claude resume  
...
コード: `presence-ui/.../gw_silent.py`、`reading_prompts.py`（`PAUSE_RESPONSE_SCHEMA`, `FELT_HINTS`）
...
- **次**: Claude `--resume` 経路（チャット直後 KV 再利用）、GW-S2 OL-GATE。
- **環境**: `PRESENCE_GW_S1_ENABLED=1`（既定）、`PRESENCE_GW_S1_TIMEOUT=90`。
