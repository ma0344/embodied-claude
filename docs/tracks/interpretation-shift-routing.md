...
**状態**: 📋 spec 確定 · **TEMP-4 ✅** · 実装は **SHIFT-R1 から**  
...
```
① TEMP-4 ✅ — plan.must_include から shift 全文 hold を外す（blast radius 削減）
② SHIFT-R1 — gateway 自動追記の縮小 or 停止（boundary 専用など短期 E）
③ SHIFT-R2 — Stage2 `correction_target` 分類 + ルーティング（TEMP-C 同型 · 中期 D · [S2 増設ルール](./utterance-anchoring.md#stage-2-を増やす判断ルール) 必須）
```

**TEMP-4 完了** — 次は SHIFT-R1（gateway 自動追記の見直し）。
...
| **SHIFT-R1** | gateway `infer_interpretation_shifts` 縮小 or 停止 | TEMP-4 推奨 |
| **SHIFT-R2** | Stage2 `correction_target` + store ルーティング | TEMP-C3 · [prottypemarkdown.md](../../prottypemarkdown.md) |
...
| 箇所 | 役割 |
|------|------|
| `presence-ui/src/presence_ui/heartbeat/interpretation_shift.py` | BIO-7 自動追記（要 SHIFT-R1） |
