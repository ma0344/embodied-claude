...
**状態**: ✅ OL5-a/b/c 運用確認済 · ✅ **OL6**（2026-06-29）· 📋 OL-STALE 計画済  
...
| **OL-STALE** | **日跨ぎで閉じない** loop のルート（`stale_policy`） | 📋 | TEMP-C4 · correction |
| **TEMP-C4** | loop 作成時 **`resolved_date` / when 継承**の固定 | 📋 | TEMP-C3 ✅ |
...
- **会話ターン跨ぎ（OL6）**: 現状 **未対応** — 下記 § OL6
- **日跨ぎ stale（OL-STALE）**: 現状 **全 open loop が対象**（`resolved_date` / topic から）— 例外ルートは § OL-STALE
...
| **OL-STALE** | `stale_policy` · 日跨ぎ exempt ルート | 📋 |
