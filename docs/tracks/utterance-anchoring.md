...
**状態**: 📋 TEMP-1/2/3/4 ✅ · TEMP-C3/C4 ✅ · **TEMP-C5 📋**（clock when · e4b）· 要 `PRESENCE_GW_S2_STAGED=1`  
...
**限界（C4 の外）**: **時刻のみ**・**「この後 午前2時」** 等 — `resolved_date` は付かない → [TEMP-C5](#temp-c5--clock--相対時刻アンカー設計--2026-06-29)。
...
### TEMP-C5 — clock / 相対時刻アンカー（設計 · 2026-06-29）

**状態**: 📋 設計メモ（e4b 手動 POC 済 · gateway 未配線）

**きっかけ（2026-06-29 夜）**: 「この後 午前2時から … サッカー … 見る」→ open loop は **望ましく作成**されたが `detail` は:

- `temporal_phrase`: `この後 午前2時`（文字列のみ）
- `resolved_date` / `start_at`: **なし**
- `until_phrase`: なし → OL6 の post-deadline も効かない

発話 `as_of` ≈ **2026-06-29 21:59 JST** なら、意味上の開始は **2026-06-30 02:00 JST**。

#### 相対マーカーを増やし続けない

| 方式 | 限界 |
|------|------|
| regex / 辞書で「この後」「あとで」「今夜」…を足す | **終わりがない** · 午前/午後/24h 跨ぎ · 文脈依存 |
| **TEMP-C4**（カレンダー日） | 明日・N月N日・来週火曜 — **日付**は決定的に足せる |
| **TEMP-C5**（時刻） | **`as_of` + when フレーズ → e4b JSON** — マーカー列挙に頼らない |
...
```
今の日時は日本時間2026年6月29日21時59分です。
「この後 午前2時」という言葉から正しい日時を作成してください。
出力: {"datetime":"yyyy-mm-dd hh:mm JST"}
```

| 入力 | e4b 出力 |
|------|----------|
| as_of 6/29 21:59 JST · `この後 午前2時` | `2026-06-30 02:00 JST` ✅ |

#### ハイブリッド（推奨）

```
loop 作成時 when_phrase あり
  → TEMP-C4 anchor_temporal_in_text（日付のみ）
  → resolved_date 取れたら終了
  → まだ null & (clock っぽい | Tier1 miss)
      → e4b TEMP-C5（as_of JST + when_phrase + 元発話要約）
      → parse datetime → detail.start_at (ISO+tz) · resolved_date (日付部分)
```

**regex の役割**: 「e4b を呼ぶか」の **軽いゲート**（`午前|午後|\d{1,2}時|この後|あとで`）— **意味解釈は e4b**。

#### 保存（案）

```json
{
  "when_phrase": "この後 午前2時",
  "start_at": "2026-06-30T02:00:00+09:00",
  "when_anchor_source": "e4b",
  "resolved_date": "2026-06-30"
}
```

- `start_at` — OL6 拡張 · compose「いつから？」· GAPI 照合の根拠
- `when_anchor_source`: `regex` | `e4b` — デバッグ用

#### 他との関係

| 層 | 役割 |
|----|------|
| **TEMP-C4** | カレンダー日 · topic 前置 |
| **TEMP-C5** | 時刻 · 日時 |
| **OL6** | `until_phrase` または将来 `start_at` + ヒューリスティックで check-in |
| **OL7** | close（視聴終了）— start_at 無くても可 |
| **reminder `_parse_clock`** | リマインド専用 · **OL when には未接続** — C5 で統合候補 |

#### 実装タッチ（着手時）

| 層 | 内容 |
|----|------|
| `presence_ui/gateway/temp_c5_when_anchor.py` | e4b prompt · parse · `as_of` 注入 |
| `apply_ol_gate_decision` / `temp_c_staged` | loop 作成直前に C5 呼び出し |
| env | `PRESENCE_TEMP_C5_ENABLED=0` · `PRESENCE_CLASSIFIER_MODEL=e4b` |

#### 受け入れ（着手時）

1. as_of 6/29 22:00 JST · `この後 午前2時` → `start_at` = 6/30 02:00 JST
2. `明日 10時` → C4 または C5 · `resolved_date` = 6/30
3. `来週中` → C4 ambiguous · **C5 を呼ばない**（needs_date_confirmation）
4. e4b 失敗 → loop は作る · `start_at` 無し（現状と同じ · 悪化しない）
...
⑪ TEMP-C5 📋 — clock when · e4b + as_of（相対マーカー列挙しない）— § TEMP-C5
⑫ OL7 📋 — return-signal close → [ol5.md](./ol5.md)
```
