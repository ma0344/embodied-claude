...
| 案 | 理由 |
|----|------|
| **bare greeting 正規表現の拡張** | 「おはよ」「やあ」… **入口の無限リスト** · 根が shift `must_include` と stale 未判定 |
...
| **TEMP-4** | `plan.must_include` — shift 全文 hold をやめ · temporal question / GAPI 時のみ schedule surface | ✅ `append_shift_plan_constraints` |
| **TEMP-5** | dream_digest · relevant_memories · remember — 同型アンカー | 💤 後 |
...
- bare greeting 拡張に頼らない
- shift あり + resolved **stale** → compose inject なし（TEMP-2）
- **通常ターン**: `must_include` は topic +「do not regress」だけ — **OLD/NEW 全文 hold しない**
- **「今日の予定は？」** 等 `is_temporal_question` + schedule-like shift → shift から schedule snippet を載せ可
- 詳細は compose の `[interpretation_shifts]` 節を参照
...
| 6/28 朝「おはよ」 | **入浴介助・角煮を今日の予定として言わない** |
| 6/28 「今日の予定は？」 | anchored past は出さない · GAPI 未接続時は正直に弱い根拠 |
| 6/27 夜「明日角煮」→ 6/28 朝 | inject は「今日角煮」相当（TEMP-3 後）または resolved==as_of で surface 可 |
...
| **理解更新**（SHIFT-R2 · 📋） | `correction` / boundary 系 | `correction_target` · canonical topic | shift · boundary · remember |
...
Gateway は **events ごと** に `resolved_date` を取り、open loop は **B（角煮）だけ** 等ポリシーで選べる（別判断）。
...
`restart-presence-ui.ps1` で反映。ingest 後 `temp_c_staged.py` → Stage1 →（greeting/other なら停止）→ Stage2 → events[] → open loop（**action あり future のみ** · 入浴介助のみは OL 作らない）。
...
```
① TEMP-1/2 ✅ — shift anchor + stale inject（presence-ui 内蔵 orchestrator）
② TEMP-C3 ✅ — Stage1→2 パイプ · events → OL（`PRESENCE_GW_S2_STAGED=1`）
③ TEMP-3 ✅ — compose inject で具体日 → 今日/明日 relativize（`prepare_shifts_for_inject`）
④ TEMP-4 ✅ — plan hold 整理（`append_shift_plan_constraints`）
⑤ TEMP-5 ～ — dream_digest / memories · shift←events
⑥ SHIFT-R — [interpretation-shift-routing.md](./interpretation-shift-routing.md)（TEMP-4 後 · 次）
```
