...
**方向**: モデルスロット整理のため **vision も `google/gemma-4-e4b` へ**（前頭葉 classifier と同族）。**2026-06-29 ma-home 本番切替**。

| Phase | 内容 | 状態 |
|-------|------|------|
| **PFC-1** | `PRESENCE_CLASSIFIER_MODEL=e4b` · Stage1/2/correction | ✅ |
| **VIS-e4b POC** | 同一 JPEG · 同一 prompt で Qwen vs e4b caption 比較 | ✅ 条件付き → [verdict](../../benchmarks/vis-e4b-poc-verdict-2026-06-29.md) |
| **VIS-12b** | 全 describe → 12b-qat · `LM_STUDIO_VISION_MODEL` 廃止 | ✅ 2026-07-06 |
| **VIS-e4b-qat ship** | （superseded by VIS-12b） | — |
...
| 5 | コード変更: `vision_reload` ラベル（旧 `qwen_reload`） | ✅ 2026-06-29 |
...
### BIO-8 と describe / multimodal の分離（2026-07-06）

| 経路 | `body_state` の目 |
|------|-------------------|
| **キャプチャ失敗** / corrupt | `failed` / affliction |
| **describe 失敗**（`VISION_DESCRIBE_FAILED`） | **変化なし**（盲目扱いしない） |
| **describe 成功**（caption あり） | `note_organ_ok` |
| **VIS-SD** 会話 multimodal 成功 | `note_eyes_multimodal_see_ok` |

`somatic.eye_affliction_summary` はキャプチャ経路のみ。会話で見えたのに describe 失敗で「目が悪い」と注入される矛盾を解消。

**未決（MEM-8）**: 見えた情報をどの層にどう encode するか — OBS-TICK / 変化時 describe 参照。
