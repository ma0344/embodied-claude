...
def test_resolve_doc_for_turn_title_then_sticky_decay(monkeypatch):
    ...
    got, reason = doc_prefetch.resolve_doc_for_turn("グループホーム本の話しよ", session)
    assert got == doc_id and reason == "title"

    # 2) no cue, sticky continues (reason=sticky), remaining 2→1
    got, reason = doc_prefetch.resolve_doc_for_turn("それでね、どう思う？", session)
    assert got == doc_id and reason == "sticky"

    # 3) sticky continues, remaining 1→0
    got, reason = doc_prefetch.resolve_doc_for_turn("うんうん", session)
    assert got == doc_id and reason == "sticky"

    # 4) sticky exhausted → none
    got, reason = doc_prefetch.resolve_doc_for_turn("全然違う話", session)
...
def test_resolve_doc_for_turn_cue_uses_active(monkeypatch):
    ...
    got, reason = doc_prefetch.resolve_doc_for_turn("さっきの本の続き", "sess-2")
