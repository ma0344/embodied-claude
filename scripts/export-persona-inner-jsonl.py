"""Export inner-voice persona LoRA training JSONL (RP-2c)."""
...
from presence_ui.training.persona_export import load_soul_core_text  # noqa: E402
from presence_ui.training.persona_inner_export import export_persona_inner_jsonl  # noqa: E402


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Export inner-voice pairs for persona LoRA (RP-2c).",
    )
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
        default=None,
        help="Candidates JSONL path (default: koyori-persona-inner-candidates.jsonl)",
    )
    ...
    candidates_path = args.output or persona_inner_candidates_jsonl_path()
    ...
    stats = export_persona_inner_jsonl(
        repo_root=args.repo,
        output_path=candidates_path,
        max_rows=args.max_rows,
    )
    curation = apply_persona_inner_curation(
    ...
    print(f"curated:    {curated_path}")
    print(
        f"rows:       {stats.pairs_written} candidates, "
